...
func SetupAuthorize(skipper echoMiddleware.Skipper, permissionManager permissions.IPermissionManager) echo.MiddlewareFunc {
	// Defaults
	if skipper == nil {
	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			grantedPermissions, err := permissionManager.GetGrantedPermissions(ctx, userId)
			if err != nil {
				logger.Logger.Error("Error when getting granted permissions: ", zap.Error(err))
			}
