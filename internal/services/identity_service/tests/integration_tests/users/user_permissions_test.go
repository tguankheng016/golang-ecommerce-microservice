...
import (
	...
	appPermissions "github.com/tguankheng016/go-ecommerce-microservice/internal/pkg/permissions"
...
func (suite *UserTestSuite) TestUserWithPermissions() {
	...
	for _, tt := range tests {
		...
		resp, err := suite.Client.R().
			...
			SetAuthToken(token).
			Get(fmt.Sprintf("/api/v1/identities/user/%d/permissions", tt.userId))
		...
		resp, err = suite.Client.R().
			...
			SetAuthToken(token).
			Put(fmt.Sprintf("/api/v1/identities/user/%d/permissions", tt.userId))
		...
		resp, err = suite.Client.R().
			SetContext(suite.Ctx).
			SetAuthToken(token).
			Put(fmt.Sprintf("/api/v1/identities/user/%d/reset-permissions", tt.userId))
